...
документа (Shadow DOM). Они опираются на концепции, которые были описаны в 
...
теневую модель документа (а он его не поддерживает), вы увидите «Название 
теневого дерева».
...
* [На этой странице есть и другие теги `h3`][4], но селектору с условием `h3` 
...
влияют на контент внутри теневого дерева. Это потому что **применение селекторов 
не переходит границы теневого дерева**.
...
[Эт-правило `@host`][5] позволяет выбрать и стилизировать элемент, который 
...
стилевой атрибут, определённый для элемента `host`. Кроме того, `@host` работает 
только в контексте корневого элемента теневого дерева, так что его нельзя 
использовать за пределами теневого дерева. 
...
Также `@host` может пригодится при стилизации нескольких ведущих элементов в 
...
используются для стилизации внутренних компонентов родных элементов браузера. 
...
Так же, как разработчики браузеров предоставляют возможность прописания стилей 
для внутренних компонентов браузеров, авторы контента в теневом дереве могут 
выделить некоторые элементы, стиль которых может быть изменён посторонними. Это 
...
привязку к соответствующему элементу в теневом дереве и отводит для посторонних 
...
применять стили з помощью внешнего CSS, но на них нельзя воздействовать с 
...
Поддержку переменных в CSS можно активировать в Chrome в разделе «Активация 
экспериментальных функций» на странице `about:flags`.

Переменные в CSS — это ещё один эффективный способ управления стилями. По сути 
это создание своеобразных «стилевых заполнителей», наполнение которых может быть 
изменено посторонними. 

Возможный сценарий использования: разработчик настраиваемого элемента обозначает 
переменные-заполнители в теневом дереве. Один для стилизации шрифта кнопки 
виджета, а другой — для изменения его цвета:
...
Самый яркий пример — это виджет для оставления комментариев. Большинство 
...
которую происходит вставка, посредством наследования шрифтов, цвета, высоты 
строки и т.д.
...
теневого дерева наследовать внешний вид других элементов `h3`, которые прописаны 
...
Если вы не уверены какие свойства наследуют родительские значения в CSS, 
взгляните на [этот удобный список][12] или поставьте птичку напротив «Показать 
...
## Стилизация совместных узлов

`.applyAuthorStyles/.resetStyleInheritance` предназначены строго для управления 
стилями узлов, заданных в теневом дереве.

С совместными узлами всё иначе. Согласно логике, они не принадлежат теневому 
дереву, они — дочерние элементы ведущего узла, которые вставляются на место в 
«момент отображения». Закономерно, они принимают стиль из страницы, на которой 
...
Если совместные узлы являются дочерними для ведущего элемента теневого дерева, 
как тогда можно к ним обратиться и стилизировать *изнутри* теневого дерева? 
Псевдоэлемент `::distributed()` служит ответом. Это первый *функциональный* 
псевдоэлемент, который принимает CSS-селектор в качестве параметра. 
...
Теневая модель документа даёт нам возможность инкапсулировать стили и впускать 
внутрь теневого дерева ровно столько внешнего воздействия, сколько нам нужно. 
Определив настраиваемые псевдоэлементы или добавив переменные-заполнители, 
разработчики могут снабдить третью сторону удобной возможностью управлять 
